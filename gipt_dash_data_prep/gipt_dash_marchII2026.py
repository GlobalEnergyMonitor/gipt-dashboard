...
tmp.to_json("./public/assets/data/gipt_data_ticker_marII2026.json", orient="records", indent=2, force_ascii=False)
...
json_output_path = "./public/assets/data/operating_plants_map_mar2026_30k.json"
...
res=[]
for tech in ['oil/gas','wind','utility-scale solar','hydropower','nuclear','bioenergy','geothermal']:
...
df_cleaned=df_cleaned[(df_cleaned.Year>=2000)&(df_cleaned.Year<2026)]
df_cleaned.to_csv('C:/Users/james/Documents/GitHub/gipt-dashboard/public/assets/data/GEM_capacity_stacked_marII2026.csv',encoding='utf-8-sig')

# Pretty-printed JSON
df_cleaned.to_json("C:/Users/james/Documents/GitHub/gipt-dashboard/public/assets/data/GEM_capacity_stacked_marII2026.json", orient="records", indent=2, force_ascii=False)
...
for country in all_countries:
	to_save=(gipt[(gipt['Country/area']==country)&(gipt.Status.isin(['operating']))].groupby(['Type','Start year'])['Capacity (MW)'].sum().unstack().T/1000.)['2000':'2026']
	...
	to_save['Net non-fossil']=to_save[[col for col in ['Bioenergy added','Geothermal added','Hydropower added','Nuclear added','Utility-scale solar added','Wind added'] if col in to_save.columns]].sum(axis=1)+to_save[[col for col in ['Bioenergy retired','Geothermal retired','Hydropower retired', 'Nuclear retired', 'Utility-scale solar retired', 'Wind retired'] if col in to_save.columns]].sum(axis=1)
...
for reg in regions:
	names=list(iea_reg_map[iea_reg_map[reg]=='Yes'].gem_name.unique())#
	to_save=(gipt[(gipt['Country/area'].isin(names))&(gipt.Status.isin(['operating']))].groupby(['Type','Start year'])['Capacity (MW)'].sum().unstack().T/1000.)['2000':'2026']
	columns=[i.capitalize() for i in to_save.columns]
	columns=[i+' added'for i in columns]
	to_save.columns=columns
...
#GLOBAL VALUES:
to_save=(gipt[(gipt.Status.isin(['operating']))].groupby(['Type','Start year'])['Capacity (MW)'].sum().unstack().T/1000.)['2000':'2026']
columns=[i.capitalize() for i in to_save.columns]
columns=[i+' added'for i in columns]
to_save.columns=columns
...
to_save['Net non-fossil']=to_save[['Bioenergy added','Geothermal added','Hydropower added','Nuclear added','Utility-scale solar added','Wind added']].sum(axis=1)+to_save[['Bioenergy retired','Geothermal retired','Hydropower retired', 'Nuclear retired', 'Utility-scale solar retired', 'Wind retired']].sum(axis=1)
to_save['Region']='World'
res_country.append(to_save)


to_save=pandas.concat([pandas.concat(res_region),pandas.concat(res_country)])[['Region','Net fossil','Net non-fossil','Coal added','Oil/gas added','Utility-scale solar added','Wind added','Hydropower added','Nuclear added','Bioenergy added','Geothermal added','Coal retired','Oil/gas retired','Utility-scale solar retired','Wind retired','Hydropower retired','Nuclear retired','Bioenergy retired','Geothermal retired']]
...
to_save.rename(columns={'Region':'Country','Oil/gas added': 'Oil and gas', 'Oil/gas retired': 'Oil and gas retired','Utility-scale solar added':'Utility-scale solar','Utility-scale solar retired':'Utility-scale solar retired',
...
full_years = pd.Series(range(2000, 2026), name='Year')
...
df_full.sort_values(by=['Country', 'Year']).fillna(0.).to_csv("./public/assets/data/net_capacity_marII2026.csv",index=False,encoding='utf-8-sig')

# Pretty-printed JSON
df_full.to_json("./public/assets/data/net_capacity_marII2026.json", orient="records", indent=2, force_ascii=False)
...
#Country
types=['coal', 'bioenergy', 'hydropower', 'geothermal', 'wind', 'utility-scale solar','nuclear', 'oil/gas']
for country in all_countries:
...
tmp.loc[tmp.Source=='utility-scale solar','Source'] = 'Utility-scale solar'
tmp.loc[tmp.Source=='wind','Source'] = 'Wind'

tmp.to_json("./public/assets/data//gipt_development_marII2026.json", orient="records", indent=2, force_ascii=False)
tmp.fillna(0.).to_csv("./public/assets/data//gipt_dev_marII2026.csv",encoding='utf-8-sig')


################################################################################################################################################
##7: FOSSIL / NON-FOSISL SPLIT
...
tmp.to_json("./public/assets/data/gipt_fossil_nonfossil_marII2026.json", orient="records", indent=2, force_ascii=False)
...
tmp.fillna(0.).to_csv("./public/assets/data/gipt_share_marII2026.csv",encoding='utf-8-sig')
...
final_df_sorted.to_csv('./public/assets/data/age_breakdown_marII2026.csv',index=False,encoding='utf-8-sig')

# Pretty-printed JSON
final_df_sorted.to_json("./public/assets/data/age_breakdown_marII2026.json", orient="records", indent=2, force_ascii=False)
...
owners_df_operating=owners_df[owners_df.Status=='operating'][['Country/area','Parent(s)','Capacity (MW)','Technology']]

#RENAME OIL/GAS AND SOLAR
owners_df_operating['Technology'] = owners_df_operating['Technology'].replace({
    'Oil/Gas': 'Oil and gas',
    'Utility-Scale Solar': 'Utility-scale solar',
})
...
# 1) aggregate to owner level first (skip this step only if already one row per owner-country-tech)
owner_totals = (
    owners_df_operating
    .groupby(['Country/area', 'Technology', 'Parent(s)'], as_index=False)['Capacity (MW)']
    .sum()
)

# 2) sort within each country-tech group
owner_totals = owner_totals.sort_values(
    ['Country/area', 'Technology', 'Capacity (MW)'],
    ascending=[True, True, False]
).copy()

# 3) mark top 20 within each country-tech
owner_totals['rank_within_group'] = (
    owner_totals.groupby(['Country/area', 'Technology']).cumcount() + 1
)

top_20 = owner_totals[owner_totals['rank_within_group'] <= 20].copy()
remaining = owner_totals[owner_totals['rank_within_group'] > 20].copy()

# 4) collapse the rest into Others
others = (
    remaining
    .groupby(['Country/area', 'Technology'], as_index=False)['Capacity (MW)']
    .sum()
)
others['Parent(s)'] = 'Others'
...
final_df = pd.concat(
    [
        top_20[['Country/area', 'Technology', 'Parent(s)', 'Capacity (MW)']],
        others[['Country/area', 'Technology', 'Parent(s)', 'Capacity (MW)']]
    ],
    ignore_index=True
)
...
sorted_df = sorted_df[~((sorted_df['Country/area'] == 'World') & (sorted_df['Parent(s)'].isin(['unknown','other','Unknown','Other','Others','others'])))]

#HARMONISE OTHERS/UNKNOWN
sorted_df['Parent(s)'].replace({
    'unknown': 'Unknown',
    'other': 'Others',
    'Other': 'Others',
    'others': 'Others'
...
sorted_df = sorted_df.rename(columns={
    'Parent(s)': 'Parent',
    'Capacity (MW)': 'Capacity (GW)',
    'Country/area': 'Country',
    'Technology': 'Type',
})


sorted_df[['Country','Parent', 'Capacity (GW)', 'Type']].to_csv('./public/assets/data/ownership_marII2026.csv',encoding='utf-8-sig', index=False)

# Pretty-printed JSON
sorted_df.to_json("./public/assets/data/ownership_marII2026.json", orient="records", indent=2, force_ascii=False)
...
pandas.concat([res_world.to_frame().T,pandas.concat(res_regs,axis=1).T,pandas.concat(res_country,axis=1).T]).fillna(0.)[['Country/Area','coal', 'oil/gas', 'solar', 'wind', 'hydropower','nuclear', 'bioenergy', 'geothermal']].to_csv("C:/Users/james/Documents/GitHub/gipt-dashboard/public/assets/data/gipt_operating_sept2025_v1.csv",encoding='utf-8-sig')

json_out=pandas.concat([res_world.to_frame().T,pandas.concat(res_regs,axis=1).T,pandas.concat(res_country,axis=1).T]).fillna(0.)[['Country/Area','coal', 'oil/gas', 'solar', 'wind', 'hydropower','nuclear', 'bioenergy', 'geothermal']]
...
pandas.concat([res_world.to_frame().T,pandas.concat(res_regs,axis=1).T,pandas.concat(res_country,axis=1).T]).fillna(0.)[['Country/Area','coal', 'oil/gas', 'solar', 'wind', 'hydropower','nuclear', 'bioenergy', 'geothermal']].to_csv("C:/Users/james/Documents/GitHub/gipt-dashboard/public/assets/data/gipt_construction_sept2025_v1.csv",encoding='utf-8-sig')
...
# -> or write to file
with open("C:/Users/james/Documents/GitHub/gipt-dashboard/public/assets/data/gipt_construction_sept2025_v1.json", "w", encoding="utf-8") as f:
    json.dump(records, f, indent=2, ensure_ascii=False)
...
# grouped_by_age.to_csv('C:/Users/james/Documents/GitHub/gipt-dashboard-V2-dev/gipt-dashboard-main/age_breakdown_v4.csv',index=False,encoding='utf-8-sig')
...
# #rename columns
# #pivoted_by_type_sorted.columns=['Country/area', 'Age Category', 'Bioenergy', 'Coal', 'Geothermal','Hydropower', 'Nuclear', 'Oil and gas', 'Utility-scale solar', 'Wind']
# #
# pivoted_by_type_sorted.to_csv('C:/Users/james/Documents/GitHub/gipt-dashboard-V2-dev/gipt-dashboard-main/age_breakdown_v2.csv',index=False,encoding='utf-8-sig')
