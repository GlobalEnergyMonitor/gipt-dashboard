"""Prepare the August 2026 III GIPT data for the dashboard outputs."""
...
GIPT_FILE = REPOSITORY_ROOT / "Global Integrated Power August 2026 III.xlsx"
...
WIND_MINIMUM_CAPACITY_MW = 10
OIL_GAS_MINIMUM_CAPACITY_MW = 50
OIL_GAS_EU_UK_MINIMUM_CAPACITY_MW = 20
...
# Apply the public tracker minimum-size thresholds before creating any
# dashboard summaries. Exact-threshold units are retained.
wind_below_threshold = (
    gipt["Type"].eq("wind")
    & gipt["Capacity (MW)"].lt(WIND_MINIMUM_CAPACITY_MW)
)

eu_uk_countries = set(
    gipt_regions.loc[
        gipt_regions[GROUP_MEMBERSHIP_COLUMNS["EU27"]].eq(1),
        REGION_COUNTRY_COLUMN,
    ]
)
eu_uk_countries.add("United Kingdom")

is_oil_gas = gipt["Type"].eq("oil/gas")
is_eu_uk = gipt["Country/area"].isin(eu_uk_countries)
oil_gas_below_threshold = is_oil_gas & (
    (
        is_eu_uk
        & gipt["Capacity (MW)"].lt(OIL_GAS_EU_UK_MINIMUM_CAPACITY_MW)
    )
    | (
        ~is_eu_uk
        & gipt["Capacity (MW)"].lt(OIL_GAS_MINIMUM_CAPACITY_MW)
    )
)

wind_rows_removed = int(wind_below_threshold.sum())
wind_capacity_removed = gipt.loc[
    wind_below_threshold, "Capacity (MW)"
].sum()
oil_gas_rows_removed = int(oil_gas_below_threshold.sum())
oil_gas_capacity_removed = gipt.loc[
    oil_gas_below_threshold, "Capacity (MW)"
].sum()

gipt = gipt.loc[
    ~(wind_below_threshold | oil_gas_below_threshold)
].copy()

remaining_wind_below_threshold = (
    gipt["Type"].eq("wind")
    & gipt["Capacity (MW)"].lt(WIND_MINIMUM_CAPACITY_MW)
)
remaining_oil_gas_below_threshold = gipt["Type"].eq("oil/gas") & (
    (
        gipt["Country/area"].isin(eu_uk_countries)
        & gipt["Capacity (MW)"].lt(OIL_GAS_EU_UK_MINIMUM_CAPACITY_MW)
    )
    | (
        ~gipt["Country/area"].isin(eu_uk_countries)
...
if (
    remaining_wind_below_threshold.any()
    or remaining_oil_gas_below_threshold.any()
):
    raise ValueError("GIPT minimum-size threshold filtering failed")

print(
    f"Wind below {WIND_MINIMUM_CAPACITY_MW} MW excluded: "
    f"{wind_rows_removed:,} rows ({wind_capacity_removed:,.3f} MW)"
)
print(
    "Oil/gas below 20 MW in EU/UK or below 50 MW elsewhere excluded: "
    f"{oil_gas_rows_removed:,} rows ({oil_gas_capacity_removed:,.3f} MW)"
)
...
if gipt_ids.isna().any() or not gipt_ids.is_unique:
    raise ValueError(
        "GIPT unit/phase IDs must be complete and unique before solar "
        "matching: "
        f"missing={int(gipt_ids.isna().sum())}, "
        f"duplicates={int(gipt_ids.duplicated().sum())}"
    )

solar_id_set = set(solar_ids)
gipt_solar_id_values = gipt.loc[
    gipt["Type"].eq("utility-scale solar"),
    "GEM unit/phase ID",
]
gipt_solar_ids = set(gipt_solar_id_values)
missing_from_gipt = sorted(solar_id_set - gipt_solar_ids)
...
if missing_from_gipt or missing_from_solar:
    raise ValueError(
        ...
        f"GSPT IDs absent from GIPT solar rows={len(missing_from_gipt)}, "
